...
create table if not exists 학생(
	학생번호 int, -- auto_increment는 기본키에만 설정할 수 있음.(자동증가 설정)
    학년 	int not null default 1,
    반 	int not null default 1,
    번호 	int not null default 1,
    이름 char(6), -- 이름을 최대 6자로 설정한 것임.
    primary key(학생번호)
);
...
create table if not exists 보유(
    ...
    성적번호 	int  not null,
    primary key(보유번호),
    -- foreign key(학생번호) references 학생(학생번호), -- 외래키 지정하는 법
	-- foreign key(성적번호) references 성적(성적번호),
    check(기말 >= 0 && 기말 <= 100),
	check(중간 >= 0 && 중간 <= 100),
	check(수행 >= 0 && 수행 <= 100)
);
-- auto_increment는 기본키로 지정된 속성에 설정하는 키워드이기 때문에, 테이블을 생성 후  altert를 이용하여 변경
alter table 학생
...
/* 
- 1학년 1반 2번 학생의 1학년 1학기 국어 성적은 중간 80, 기말 85, 수행 89이다.
  이 정보를 테이블에 추가하는 쿼리를 작성해보세요.
  
  1. 학생 테이블에 1학년 1반 2번 학생이 있는지 확인(있다 => 학생번호가 2번. 없으면 => 추가)
  2. 성적 테이블에 1학년 1학기 국어 성적이 있는지 확인(있다 => 성적번호가 1번. 없으면 => 추가)
  3. 보유 테이블에 학생번호가 2번이고, 성적번호가 1이고, 중간 80, 기말 85, 수행 89을 추가
*/
...
update 보유
	set
		총점 = 기말 * 0.5 + 중간 * 0.4 + 수행 * 0.1; -- 모든 조건(전체 테이블)을 가져와야 하기 때문에 where은 필요가 없음.
