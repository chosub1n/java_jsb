...
CREATE TABLE `board` (
	...
	`bd_reg_date`	datetime	NOT NULL default current_timestamp, -- current_timestamp -> 시간 자동 기입
...
-- 외래키로 지정된 기본키들은 속성을 바꾸기 힘들다. (그래서 외래키 전에 바꿔줌)
...
-- 최고 관리자를 등록하는 쿼리 아이디 : admin123 비번 : admin123, 이름 : 관리자, 번호 : 010-0000-0000
insert into user(us_id, us_pw, us_name, us_phone, us_authority)
	values('admin123', 'admin123', '관리자', '010-0000-0000', 10);
...
-- 1. 공지사항은 최고 관리자만 작성할 수 있고, 회원이상만 읽을 수 있다.
-- 2. 일반은 회원이상 작성할 수 있고, 비회원도 읽을 수 있다.
-- 3. 문의사항은 회원이상 작성할 수 있고, 회원 이상만 읽을 수 있고, 비밀글을 작성할 수 있다.
insert into category(ca_name, ca_r_authority, ca_w_authority, ca_secret_pos)
	values('공지사항', 1, 10, 0), ('일반', 0, 1, 0), ('문의사항',1 ,1, 1);
    
-- abcd1234회원이 일반 게시글에 제목 : 안녕하세요. 내용 : 만나서 반갑습니다. 라는 글을 등록했다.
-- 최고관리자가 공지사항에 제목 : 공지사항1 내용 : 공지사항입니다. 라는 글을 등록했다.
-- qwer1234회원이 문의 게시글에 제목 : 문의합니다. 내용 : 비번을 바꾸고 싶어요 라는 글을 비밀글로 등록했다.
insert into board(bd_ca_name, bd_title, bd_content, bd_us_id, bd_secret)
...
-- update문을 사용하지 않고 bd_ori_num에 값을 넣어주는 쿼리 (select를 이용함)
-- ifnull(max(bd_num),0) -> null인 경우에 0을 넣어주세요. count는 이렇게 안 해줘도 됨.
insert into board(bd_ca_name, bd_title, bd_content, bd_us_id, bd_secret, bd_ori_num)
	select '일반', '안녕하세요1.', '만나서 반갑습니다1.', 'abcd1234', 0, ifnull(max(bd_num),0)+1
    from board;

-- admin123관리자가 3번 게시글에 답글을 남김. 제목 : 문의하신 내용에 답변입니다. 내용 : 비번찾기를 이용하세요.
insert into board(bd_ca_name, bd_title, bd_content, bd_us_id, bd_secret, bd_ori_num, bd_depth)
		select '문의사항', '문의하신 내용에 답변입니다.', '비번찾기를 이용하세요.', 'admin123', 0,
        bd_ori_num, bd_depth+1 from board
        where bd_num = 3;

-- qwer1234회원이 5번 게시글에 답글을 남김. 제목 : 그래도 안 돼요. 내용 : 비번찾기를 이용해도 안 됩니다. 
insert into board(bd_ca_name, bd_title, bd_content, bd_us_id, bd_secret, bd_ori_num, bd_depth)
		select '문의사항', '그래도 안 돼요.', '비번찾기를 이용해도 안 됩니다.', 'qwer1234', 0,
		bd_ori_num, bd_depth+1 from board
        where bd_num = 5;
        
-- 3번 게시글을 조회했을 때(게시글 내용을 확인했을 때) 필요한 쿼리 
update board set bd_views = bd_views + 1 where bd_num = 3;

-- 추천/비추천을 했다 => 해당 게시글/댓글에 사용자가 처음 추천/비추하는 경우 => 등록, 아닌 경우 => 수정(삭제) 

-- 1번 게시글에 qwer1234회원이 추천했을 때 쿼리문(게시글을 확인하고, 추천버튼을 클릭했다고 가정) 
insert into likes(li_us_id, li_state, li_target, li_target_num) values ('qwer1234', 1, 'board', 1);

-- qwer1234회원이 1번 게시글에 추천을 한 번 더 눌러 추천을 취소했을 때 쿼리문
-- delete 사용. 정보가 아예 사라짐
delete from likes where li_us_id = 'qwer1234' and li_target = 'board' and li_target_num = 1;
-- update 사용. li_state만 변경
update likes set li_state = 0 
	where li_us_id = 'qwer1234' and li_target_num = 1 and li_target = 'board';
    
-- 일반, abcd1234, 제목입니다2, 내용입니다.
-- 첨부파일을 2개 추가해서 게시글을 등록했다. 첨부파일을 추가할 때 추가해야하는 게시글의 번호를 안다고 가정
