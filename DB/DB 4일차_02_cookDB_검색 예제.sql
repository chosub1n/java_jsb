...
-- 회원별 구매 총 금액을 조회
select userId, sum(price*amount) as `총 구매금액` from buytbl group by userId;
...
select userId, sum(price*amount) as `총 구매금액` from buytbl 
	group by userId
...
-- 각 지역에 사는 회원의 수를 조회
select addr, count(userID) as `회원 수` from usertbl group by addr;

-- 경남지역에 사는 회원이 구매한 제품 목록을 조회
