-- insert나 update 되었을 때(새로 변경되거나 추가될 때) new를 붙임
 -- 1. 처음으로 추천/비추천을 했을 때, 해당 게시글에 추천 또는 비추천을 업데이트하는 트리거
 -- likes테이블에 insert를 하는 경우 => board 테이블에서 해당 게시글에 대한 추천/비추천 update
 ...
 begin
    ...
    if _state = 1 and _table like 'board' then 
     ...
     if _state = -1 and _table like 'board' then 
		update board set bd_up = bd_down + 1 where bd_num = _num;
	end if;
