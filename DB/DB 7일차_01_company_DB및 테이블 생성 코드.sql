...
-- pa_num을 pa_num int auto_increment not null; 로 바꿀 거야
...
insert into department(de_name, de_location) values
('개발부', '본관 3층 301호'), ('영업부', '본관 2층 201호'),
('마켓팅부', '본관 2층 202호'), ('디자인부', '본관 2층 302호'),
('기타', NULL);
...
insert into pay(pa_em_num, pa_date, pa_salary, pa_name, pa_year)
	select em_num, '2022-01-05', rn_base + rn_add * em_year, em_rn_name, em_year from employee
		join `rank` on em_rn_name = rn_name
        where em_enter < '2022-01-05'; -- 2022-01-05전에 입사한 사람만 지급

-- 2022년 2월 5일에 등록된 모든 사원들의 월급을 지급했다. (1월 월급을 지급)
insert into pay(pa_em_num, pa_date, pa_salary, pa_name, pa_year)
	select em_num, '2022-02-05', rn_base + rn_add * em_year, em_rn_name, em_year from employee
		join `rank` on em_rn_name = rn_name
        where em_enter < '2022-02-05'; -- 2022-02-05전에 입사한 사람만 지급
        
-- 2022년 3월 5일에 등록된 모든 사원들의 월급을 지급했다. (1월 월급을 지급)
insert into pay(pa_em_num, pa_date, pa_salary, pa_name, pa_year)
	select em_num, '2022-03-05', rn_base + rn_add * em_year, em_rn_name, em_year from employee
		join `rank` on em_rn_name = rn_name
        where em_enter < '2022-03-05'; -- 2022-03-05전에 입사한 사람만 지급

-- 2022년 4월 5일에 등록된 모든 사원들의 월급을 지급했다. (1월 월급을 지급)
insert into pay(pa_em_num, pa_date, pa_salary, pa_name, pa_year)
	select em_num, '2022-04-05', rn_base + rn_add * em_year, em_rn_name, em_year from employee
		join `rank` on em_rn_name = rn_name
        where em_enter < '2022-04-05'; -- 2022-04-05전에 입사한 사람만 지급
