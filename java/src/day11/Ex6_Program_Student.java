...
public class Ex6_Program_Student {
	 ...
	 * 1. 기능을 주석으로 정리
	 * - 학생 정보 입력
	 *	 - 학년, 반, 번호, 국어, 영어, 수학 성적을 한 번에 입력 받음
	 *   - 입력한 학생정보를 저장
	 * - 학생 정보 출력
	 *   - 전체 학생의 학년, 반, 번호, 국어, 영어, 수학 성적을 출력
	 * - 학생 정보 수정
	 * 	 - 학년, 반, 번호를 입력받음
	...
	public static void main(String[] args) {	
		...
		for(; menu !=5; ) {
			...
			case 1:
				...
				//Ex6_Student의 생성자를 이용하여 객체를 생성한 후 학생 배열 std에서 index번지에 저장				
				std[index] = new Ex6_Student(name, grade, classNum, num, kor, eng, math);
				//index를 1증가 => 여기서 index는 현재 저장된 학생의 수
				index++;	
				break; 
			case 2: 
			...
			case 3: 
				...
				for(int i = 0; i < index; i++) {
				//입력받은 학년, 반, 번호와 일치하는 학생이 있는지 확인해서 있으면 이름, 국어, 영어, 수학 성적을 입력 받은 후
			...
			case 4:
				...
				classNum = scan.nextInt();
				num  = scan.nextInt();
				int delIndex = -1; //삭제할 학생 정보의 번지. -1을 입력한 이유는 음수 중에서 임의 값으로 작성한 것임. (보통 0보다 작은 수를 필요로 할 때 -1를 사용한다.) 
				// 반복문을 이용하여 0번지부터 index명의 학생을 비교하여 
				for(int i = 0; i < index; i++) {
				//일치하는 학생 정보가 있으면 delIndex번지인지 기억하고 반복문 종료
					if(std[i].equal(grade,classNum,num)) {
						delIndex = i;			
						break;
					}
	...
	/* 기능 : 메뉴를 출력하고 메뉴를 압력하면 입력한 메뉴를 돌려주는 메소드
	 * 매개변수 : 입력 받기 위한 Scanner => Scanner scan
	 * 리턴타입 : 입력한 메뉴 => 정수 => int
	 * 메소드명 : selectMenu*/
