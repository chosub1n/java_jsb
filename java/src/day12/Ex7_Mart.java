...
public class Ex7_Mart {
	/* 음료수/박스과자를 관리하는 마트(종류는 최대 30개 - 음료수 + 박스과자 종류 합쳐서 30개)
	 * 1. 제품 등록 (신규제품)
	 * 2. 제품 입고
	 * 3. 제품 선택
	 * 4. 제품 구매
	...
	public static void main(String[] args) {
		...
		do {
			...
			case 1:
				/* 음료수인지 박스과자인지 선택, 
				 * 수량, 제품명, 가격, 박스과자면 1박스당 개수를 음료수면 용량을 입력
				...
				case 1: case 2:
					...
					//잘못된 서브메뉴를 선택하면
					default:
						System.out.println("선택할 수 없는 상품입니다.");
				}
				break;
			//선택한 메뉴가 2이면(제품 입고)	
			case 2:
			...
			case 4:
				...
				//금액이 부족하면 결제를 취소할건지 물어봄
				if(sum > buyPrice) {
					System.out.print("결제를 취소하겠습니까?(취소시 장바구니는 비워집니다. y/n) : ");
	...
	/* 기능 : 선택한 subMenu에 맞는 제품 정보를 Scanner를 통해 입력 받아 제품 객체를 생성한 후 생성한 
	 *       제품을 알려주는 메소드
	...
	public static void returnProductList(Product list[], int listCount, Product basket[], int basketCount) {
		...
		for(int i = 0; i < listCount; i++) {
			for(int j = 0; j < basketCount; j++) {
				...
				//판매제품 이룸과 장바구니제품 이름이 같으면
				if(pi.getName().equals(pj.getName())) {
					//pi.amount: 재고량, pj.amout: 구매하려고 선택했던 수량
					//장바구니에 있는 제품을 제자리에 돌려놓음 : 판매제품 수량에 장바구니 제품을 수량 추가
