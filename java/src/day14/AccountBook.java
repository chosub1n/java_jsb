...
public class AccountBook {
	//내역의 최대 개수
	public static final int MAX = 100; //final이 붙어서 상수. 수정 불가능!
	//가계부에서 내역을 최대 100개 기록할 수 있다.
	private Item items[] = new Item[MAX];
	private int count; //현재 기록된 내역의 개수

		
		/* 기능 : 현재까지 기록된 내역을 확인하는 메소드
		 * 		items에 있는 내역들을 count에 출력하는 메소드
		 * 매개변수 : 없음
		 * 리턴타입 : 없음 => void
		 * 메소드명 : readItems	
		 * */
	public void readItems() {
		...
		for(int i = 0; i<count; i++) {
			System.out.println(i+1+". "+items[i]); //items[i]는 items[i].toString()로 호출
			
		}
	}
	/* 기능 : 가계부에 새 내역을 추가하는 메소드
	 * 		세 내역(Item)이 주어지면 items에 추가하여 추가 됐는지 알려주는 메소드
	 * 매개변수 : 새 내역 => Item item
	...
	/* 기능 : 가계부에서 기존 내역을 수정하여 수정 여부를 알려주는 메소드
	 * 
