...
public class Ex1_Wrapper {
	...
	public static void main(String[] args) {
		 ...
		 * - Wrapper클래스와 기본타입의 차이
		 *   - Wrapper클래스의 기본 객체는 기본타입값과 null을 가짐
		 *   => 언방식할 때 조심해야 함. null일 수도 있기 때문
		 *   - 기본타입은 기본타입값만 가질 수 있다.
		 * - 두 기본타입 변수는 ==로 비교 가능
		 *   -기본 타입 변수와 Wrapper클래스 객체를 ==로 비교할 수 있다.
		 *   => Wrapper클래스 객체가 자동으로 언박싱이 되어서 ==로 비교
		 * - 두 Wrapper클래스 객체는 ==로 비교하면 안 됨. (equals로 비교해야 함)
		 * */
		
		//List<Integer> list = new ArrayList<Integer>(); //Integer 자리에 int가 올 수 없음!!!!! --> 제네릭클래스
		...
		if(num7 == num8) { //.equals로 비교하면 "같다" 가 나옴.
