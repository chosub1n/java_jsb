...
import java.util.*; ///.*로 바꿈으로서 모든 임폴트를 다 가져옴

public class Ex8_List_Test2 {

	public static void main(String[] args) {
		List<Student> list = new ArrayList<Student>();
		//list.add(new Student(1,1,1, "홍길동")); //list에서는 사용할 수 있지만 외부에서는 사용 불가능
		Student std = new Student(1,1,1, "홍길동");
		list.add(std);
		/* - student클래스에 equals를 오버라이딩 하지 않으면 indexOf, contains, remove메소드가 원하는대로 동작하지 않음
		 * 해쉬코드를 설정하니까 결과 값이 false에서 true로 바뀜*/
...
class Student{
	...
	public Student(Student std) {
		...
		this.name = new String(std.name); //new String를 이용해 깊은복사를 해서 객체를 공유하지 않게 함.
	}
